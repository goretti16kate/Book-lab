...
<body>
    ...
    $reader = $_SESSION['user'];
                                    if(isset($_POST['submit']))
                                        {$s=$_POST['title'];
                                            $sql="select * from Safari_final_project.record,Safari_final_project.book where user_name = '$reader' and Date_of_Issue is NOT NULL and Date_of_Return is NULL and book.Bookid = record.BookId and (record.BookId='$s' or Title like '%$s%')";
                                        }
                                    else
